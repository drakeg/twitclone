...
def test_public_post_contract_exposes_bounded_fields(client, app):
    ...
    with app.app_context():
        ...
        db.session.flush()
        associate_topics(tweet, explicit_raw="AWS", content=tweet.content)
        db.session.commit()
        tweet_id = tweet.id
...
def test_removed_and_future_posts_fail_closed(client, app):
    ...
    with app.app_context():
        ...
        db.session.add_all([removed, future])
        db.session.commit()
        ids = (removed.id, future.id)
...
def test_space_scoped_post_is_not_exposed_by_global_public_api(client, app):
    ...
    with app.app_context():
        space = Space(
            slug="api-space",
            name="API Space",
            description="Space-scoped content",
            owner_id=user_id,
        )
        tweet = Tweet(content="space-only post", user_id=user_id)
        db.session.add_all([space, tweet])
        db.session.flush()
...
def test_public_post_endpoint_is_read_only(client, app):
    ...
    with app.app_context():
        tweet = Tweet(content="read only", user_id=user_id)
        db.session.add(tweet)
        db.session.commit()
        tweet_id = tweet.id
