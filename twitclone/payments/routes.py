...
def _stripe_ready():
    return bool(current_app.config.get('STRIPE_BILLING_ENABLED') and current_app.config.get('STRIPE_SECRET_KEY'))
...
def _sync_subscription_object(obj):
    ...
    if subscription is None:
        subscription = Subscription(user_id=user.id, plan_id=plan.id, provider='stripe', provider_subscription_id=provider_id)
    ...
    session = stripe.checkout.Session.create(
        ...
        line_items=[{
            'price_data': {
                ...
                'product_data': {'name': f'Ripple {plan.name}', 'description': plan.description or ''},
            },
            'quantity': 1,
        }],
        subscription_data={'metadata': {'ripple_user_id': str(current_user.id), 'ripple_plan_key': plan.key}},
        metadata={'ripple_user_id': str(current_user.id), 'ripple_plan_key': plan.key},
        success_url=url_for('payments.checkout_success', _external=True) + '?session_id={CHECKOUT_SESSION_ID}',
        cancel_url=url_for('payments.billing_home', _external=True),
    )
...
def customer_portal():
    if not _stripe_ready():
        flash('Billing management is not configured yet.', 'warning')
        return redirect(url_for('payments.billing_home'))
    ...
    if subscription is None:
        flash('No Stripe billing account is associated with your Ripple account yet.', 'warning')
        return redirect(url_for('payments.billing_home'))
    _configure_stripe()
    session = stripe.billing_portal.Session.create(customer=subscription.provider_customer_id, return_url=url_for('payments.billing_home', _external=True))
    return redirect(session.url, code=303)
