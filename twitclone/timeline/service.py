...
def build_timeline_posts(*, now, viewer=None, feed_mode="all", topic_slug=None):
    ...
    for tweet in Tweet.query.filter(_visible_tweet_filter(now)).all():
        if include_user(tweet.user_id) and include_tweet(tweet):
            posts.append({"id": tweet.id, "source_id": tweet.id, "action_tweet_id": tweet.id, "content": tweet.content, "timestamp": _tweet_timeline_timestamp(tweet), "edited_at": tweet.edited_at, "type": "tweet", "user": tweet.user, "image": tweet.image, "original_tweet": None, "original_user": None, "poll": None, "poll_id": None, "has_voted": False, "report_type": "tweet", "report_id": tweet.id, "report_author_id": tweet.user_id, "conversation_intent": _tweet_conversation_intent(tweet), "conversation_state": _tweet_conversation_state(tweet), "topics": _tweet_topics(tweet)})
    if feed_mode != "quiet":
    ...
    if feed_mode != "topic":
        for quote in Quote.query.join(Quote.tweet).filter(_visible_tweet_filter(now), Quote.is_removed.is_(False)).all():
            if include_user(quote.user_id) and quote.tweet_id not in scoped_tweet_ids:
                posts.append({"id": quote.id, "source_id": quote.id, "action_tweet_id": quote.tweet_id, "content": quote.content, "timestamp": quote.timestamp, "edited_at": None, "type": "quote", "user": quote.user, "image": None, "original_tweet": quote.tweet, "original_user": quote.tweet.user, "poll": None, "poll_id": None, "has_voted": False, "report_type": "quote", "report_id": quote.id, "report_author_id": quote.user_id, "conversation_intent": None, "conversation_state": None, "topics": []})
        for poll in Poll.query.filter_by(is_removed=False).all():
            ...
            posts.append({"id": poll.id, "source_id": poll.id, "action_tweet_id": None, "content": poll.question, "timestamp": poll.created_at, "edited_at": None, "type": "poll", "user": poll.user, "image": None, "original_tweet": None, "original_user": None, "poll": poll, "poll_id": poll.id, "has_voted": has_voted, "poll_is_active": poll.is_active_at(now), "report_type": "poll", "report_id": poll.id, "report_author_id": poll.user_id, "conversation_intent": None, "conversation_state": None, "topics": []})
